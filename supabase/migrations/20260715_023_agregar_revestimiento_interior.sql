-- ─── Agregar cat_revestimiento_int (revestimiento interior) al pipeline IA ───
-- Noveno catálogo soportado por la extracción IA. Es conceptualmente distinto
-- de cat_aislacion_ext (20260715_022): aislación exterior conserva temperatura
-- por fuera de la cañería, revestimiento interior protege la superficie interna
-- en contacto con el fluido (ej. engomado, epóxico interior, cemento centrifugado).
-- Redefine las mismas 3 funciones de 20260715_022 agregando la rama faltante.
...
BEGIN
  ...
  IF p_tabla_destino NOT IN (
    ...
    'cat_aislacion_ext', 'cat_porcentaje_nde', 'cat_tipo_prueba', 'cat_tipo_union', 'cat_revestimiento_int'
  ) THEN
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_tabla = 'cat_revestimiento_int' THEN
      v_desc := NULLIF(trim(v_fila.payload ->> 'descripcion'), '');
      SELECT id, descripcion INTO v_act FROM lukeapp.cat_revestimiento_int
      WHERE proyecto_id = v_lote.proyecto_id AND upper(codigo) = v_fila.clave_natural;
  ...
  FOR v_fila IN SELECT * FROM lukeapp.import_filas WHERE lote_id = p_lote_id AND aprobada AND accion IN ('nueva', 'modificada', 'ausente') ORDER BY accion, nro_fila LOOP
    ...
    ELSIF v_tabla = 'cat_revestimiento_int' THEN
      IF v_fila.accion = 'nueva' THEN
        ...
        n_ins := n_ins + 1;
      ELSIF v_fila.accion = 'modificada' THEN
        UPDATE lukeapp.cat_revestimiento_int SET descripcion = NULLIF(trim(v_fila.payload ->> 'descripcion'), ''), activo = true, actualizado_por = v_uid
        WHERE proyecto_id = v_lote.proyecto_id AND upper(trim(codigo)) = v_fila.clave_natural;
        n_upd := n_upd + 1;
      END IF;
