...
5. **Un solo lugar para acciones de página: la barra superior**. Ninguna vista debe renderizar sus propios botones de navegación/acción primaria (volver, crear, importar, eliminar…) dentro de su contenido — se registran en la barra superior con el hook `useHeaderActions()` (`src/hooks/useHeaderActions.tsx`), que expone un slot dinámico entre el logo y los controles globales (config, usuario, salir). Quedan **fuera** de esta regla — se mantienen inline donde ya están — los botones de formulario (guardar/cancelar), las acciones por fila/ítem de una lista o tabla, los pasos de un wizard, y los botones pegados a un buscador inline: mover esos a la barra superior perdería el contexto que los hace útiles.
6. **Iconos con moderación**: evita emojis decorativos como prefijo de texto en botones (ej. "📊 Importar", "✨ Procesar") — el texto ya es suficiente y varios botones con emoji distinto en la misma fila se ve ruidoso. Los íconos de `lucide-react` (monocromáticos, un solo símbolo) están bien para botones icon-only puntuales (ej. el engranaje de configuración). Emojis como estado grande y único en una pantalla vacía/de error (ej. el ✕ de "Cuenta Rechazada") no son el problema — el problema es la acumulación de emojis decorativos repetidos.
