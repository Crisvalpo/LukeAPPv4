...
5. **Un solo botón "Volver" por navegación**: la barra superior (`App.tsx`, junto al logo) ya tiene "← Proyectos" que vuelve a la cartera desde cualquier `vista`. Ninguna pantalla debe agregar su propio botón "Volver"/"Atrás" que apunte al mismo destino — es redundante y confunde. Solo se permite un botón de navegación propio dentro de una pantalla cuando vuelve a un nivel **intermedio y distinto** que la barra superior no cubre (ej. de "Revisión de lote" a "Biblioteca Documental", o un drill-down interno como el detalle de un proyecto dentro de la Cartera).
